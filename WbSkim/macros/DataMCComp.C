...
//	mc5 = TFile::Open((path + "/" + version + "/" + "QCD.root").c_str());
	...
	if (useWbb) {
	  if (title.find("_bb")!=string::npos || title.find("_2b")!=string::npos || title.find("_recoVTX")!=string::npos || title.find("_tracks")!=string::npos) {
	    if (h_mc1b) {
...
//	h_mc5 = (TH1F*)gDirectory->Get(title.c_str());
	...
	if (h_mc1b) leg->AddEntry(h_mc1b,"W+b","f");
	if (h_mc1c) leg->AddEntry(h_mc1c,"W+c","f");
	if (h_mc1c && h_mc1b) {
	  leg->AddEntry(h_mc1,"W+uds","f");
	} else {
