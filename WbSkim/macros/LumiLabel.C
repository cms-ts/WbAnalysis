...
TLatex * CMSPrel (Float_t Lumi, TString _decaychannel) {
  ...
  latexLabel->DrawLatex (0.25, 0.25, "anti-k_{T} (R = 0.5) PF Jets > 25 GeV");
...
TLatex * CMSPrel2 (Float_t Lumi, TString _decaychannel, int pos, double x, double y) {
  ...
  latexLabel3->SetTextFont(63);
  latexLabel3->SetLineWidth(3);
  latexLabel3->SetNDC();

  latexLabel3->SetTextSize(22);
  latexLabel3->SetTextColor(kBlack);
  if (pos==0)  latexLabel3->DrawLatex(0.125,0.88,"CMS");
  if (pos==1)  latexLabel3->DrawLatex(0.1,0.94,"CMS Preliminary");

  if (pos==0) {
    TLatex *latexLabel4 = new TLatex();
    latexLabel4->SetTextFont(53);
    latexLabel4->SetLineWidth(3);
    latexLabel4->SetNDC();

    latexLabel4->SetTextSize(15.2);
    latexLabel4->SetTextColor(kBlack);
    latexLabel4->DrawLatex(0.125,0.85,"Preliminary");
  }
